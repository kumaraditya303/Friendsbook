...
const reducer = (state = State, action: AuthActionType) => {
  switch (action.type) {
    case AuthAction.START:
      return {...state, error: null, loading: true, authenticated: action.authenticated};
    ...
    case AuthAction.FAIL:
      return {...state, error: action.error, loading: false, authenticated: action.authenticated};
